...
import { sendBrevoCampaign } from "../../modules/email/providers/brevo/brevo.campaigns.js";
...
export const emailCampaignWorker = new Worker(
  ...
  async (job) => {
    ...
    campaign.startedAt = new Date(); //when send begins (worker)
    await campaign.save();
...
emailCampaignWorker.on("completed", (job) => {
  console.log(`${new Date().toLocaleString()} Email campaign job completed`);
});
...
emailCampaignWorker.on("progress", (job) => {
  console.log(`${new Date().toLocaleString()}  Email campaign job in progress`);
});
